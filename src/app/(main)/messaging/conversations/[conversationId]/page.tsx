...
// This file is now located at /src/app/(main)/messaging/conversations/[conversationId]/page.tsx
'use client';
...
export default function ConversationViewPage() {
  ...
  const handleForwardMessage = (message: Message, includeAttachments: boolean) => {
    ...
    if (includeAttachments) {
        const attachmentIds = (message.attachments || []).map(att => att.id).join(',');
        if (attachmentIds) {
          query.set('forwardAttachments', attachmentIds);
        }
    }
