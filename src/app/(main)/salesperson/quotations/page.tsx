...
// src/app/(main)/salesperson/quotations/page.tsx
'use client';
...
import { PlusCircle, Search, FileSignature, Edit, Eye, Trash2, Filter, Loader2, Settings2, Send, CheckCircle, XCircle, Edit3 as RevisionIcon, PauseCircle } from 'lucide-react';
...
const QUOTATION_STATUSES_FILTER: (QuotationStatus | 'all')[] = ['all', 'draft', 'sent', 'accepted', 'rejected', 'revision', 'hold', 'converted'];

// Define valid status transitions
const VALID_STATUS_TRANSITIONS: Partial<Record<QuotationStatus, { newStatus: QuotationStatus; label: string; icon?: React.ElementType }[]>> = {
  draft: [
    { newStatus: 'sent', label: 'Mark as Sent', icon: Send },
  ],
  sent: [
  ...
  accepted: [
    { newStatus: 'hold', label: 'Put on Hold', icon: PauseCircle },
    { newStatus: 'sent', label: 'Revert to Sent', icon: Send }, // Added Revert to Sent option
  ],
  revision: [
...
export default function SalespersonQuotationsPage() {
  const { currentUser, quotations, getQuotationsForSalesperson, deleteQuotation, updateQuotation: appUpdateQuotation, loadDataFromDb, isDataLoaded } = useApp();
  ...
  useEffect(() => {
    if (currentUser) {
      setLocalQuotations(getQuotationsForSalesperson(currentUser.id));
    }
  }, [quotations, currentUser, getQuotationsForSalesperson]);
  ...
  }, [localQuotations, searchTerm, statusFilter]);
  ...
  if (!isDataLoaded && !currentUser) {
    return <div className="flex justify-center items-center h-64"><Loader2 className="h-8 w-8 animate-spin text-primary" /> <span className="ml-2">Loading quotations...</span></div>;
  }
    ...
    <div className="space-y-6">
      <Card className="shadow-md">
        <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between">
          <div>
            <CardTitle className="text-2xl flex items-center"><FileSignature className="mr-2 h-7 w-7 text-primary" /> My Quotations</CardTitle>
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Select value={statusFilter} onValueChange={(value) => setStatusFilter(value as QuotationStatus | 'all')}>
                ...
                <SelectContent>
                  {QUOTATION_STATUSES_FILTER.map(status => (
              ...
              <Table>
                ...
                <TableBody>
                  {filteredQuotations.map((q) => {
                    const possibleActions = VALID_STATUS_TRANSITIONS[q.status] || [];
                    return (
                    <TableRow key={q.id}>
